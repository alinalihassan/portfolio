...
I started a projects a few months ago at work and I was excited to make the best use of multithreading in that particular scenario using NodeJS and Typescript.
...
integrate top-level await in V8 [more than 2 years ago](https://v8.dev/features/top-level-await), albeit with some changes required in order to use it.
...
Remember that many of these settings are following this post are experimental, and they might change with time. It's likely that it will be much easier
...
Our test script in `package.json` will look similar like this:
```json
"scripts": {
